...
void workingThread_SE( unsigned int tn, unsigned int start, unsigned int end, CppSERead *workingReads,
    ...
    for( unsigned int ii=start; ii!=end; ++ii ) {
        ...
        writebuffer->b1stored[tn] += sprintf( writebuffer->buffer1[tn]+writebuffer->b1stored[tn], "%s\n%s\n+\n%s\n",
                                    workingReads[ii].id.c_str(), workingReads[ii].seq.c_str(), workingReads[ii].qual.c_str() );
    }
}
