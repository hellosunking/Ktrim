...
can find a pre-compiled executable file under `bin/` directory for Linux x86_64 system compiled using
`G++ v7.4.0`. If you could not run it (which is usually caused by low version of `libc++` library) or
you want to build a version optimized for your machine, you can re-compile the programs:
```
...
## Outputs explanation
`Ktrim` outputs the trimmed reads in FASTQ format and key statistics, including the numbers of reads with
adapters and kept in the trimmed files.
